...
import_dir <- "/Users/julianashwin/Documents/DPhil/Clean_Data/ECB_articles/vader/"
export_dir <- "data/vader/"
...
#sources <- french_sources
"
Import sources and combine into one 
"
import_sources <- function(import_dir, sources){
  df <- read.csv(paste0(import_dir, sources[1], "_vader.csv"),stringsAsFactors = FALSE)
  if (length(sources) > 1){
    for (source in sources[2:length(sources)]){
      df_in <- read.csv(paste0(import_dir, source, "_vader.csv"),stringsAsFactors = FALSE)
      df <- rbind(df, df_in)
    }
  }
...
aggregate_senti <- function(df){
  ...
  df_agg <- as_tibble(df) %>%
    mutate(ndocs = 1) %>%
    group_by(date) %>%
    summarise(vader_sum = sum(vader_sum, na.rm = T), vader_nwords = sum(vader_nwords, na.rm = T),
              ndocs = sum(ndocs, na.rm = T)) %>%
    full_join(dates_df) %>%
    mutate(vader_sum = replace_na(vader_sum, 0), vader_nwords = replace_na(vader_nwords, 0), 
           ndocs = replace_na(ndocs, 0)) %>%
    arrange(date) %>%
    group_by(quarter) %>%
    mutate(vader = vader_sum/vader_nwords,
           vader_cum = cumsum(vader_sum)/cumsum(vader_nwords)) %>%
    mutate(vader = replace_na(vader, 0), vader_cum = replace_na(vader_cum, 0))
...
write.csv(french_daily, paste0(export_dir, "french_vader.csv"),row.names = FALSE)

french_daily %>%
  mutate(month = floor_date(date, unit = "months")) %>%
  group_by(month) %>%
  summarise(vader_sum = sum(vader_sum), nwords = sum(vader_nwords)) %>%
  mutate(vader = vader_sum/nwords) %>%
  ggplot(aes(x = month, y = vader)) + geom_line()


## German
german_articles <- import_sources(import_dir, german_sources)
german_daily <- aggregate_senti(german_articles)
write.csv(german_daily, paste0(export_dir, "german_vader.csv"),row.names = FALSE)
...
write.csv(italian_daily, paste0(export_dir, "italian_vader.csv"),row.names = FALSE)
...
write.csv(spanish_daily, paste0(export_dir, "spanish_vader.csv"),row.names = FALSE)
